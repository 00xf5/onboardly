...
### 4. Financial Interface (Crypto-Native)
The platform features a built-in **NOWPayments** portal, allowing for seamless crypto-relays and tier upgrades.
- **Tier Escalation:** Move from the baseline *Nexus* tier to *Pro* ($99/mo) and *Agency* ($199/mo).
...
- **Recent Events Feed:** A dedicated stream of partner activity (Task completions, skips, failures).
- **Actionable Recommendations:** Smart insights that highlight which onboarding steps are actually blocking your growth.

## 💻 Technology Stack

- **Frontend:** React + Vite + Tailwind CSS (Aesthetic: Cypherpunk/Modern Dark Mode)
...
3. Establish your environment variables in `.env`:
   ...
   NOWPAYMENTS_API_KEY=74RC1SY-BR9MZP8-QRTYVG3-RDYBB02
   ```
...
- `/src/pages/PublicOnboarding.tsx`: The bridge for inbound partners.
- `/src/components/ui/`: High-aesthetic, low-friction UI components.
- `/src/lib/firebase.ts`: The bridge to the persistent database.
